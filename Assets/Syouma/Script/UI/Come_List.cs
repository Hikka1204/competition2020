...

{
    ...
    //string Previous = null; // ひとつ前のコメントを入れる変数

    [SerializeField] GameObject Comment_Manager;
    ...
    void Start()
    {
        ...
        // コメントする人をランダムで決める
        NameNum = Random.Range(0, C_manager.Audience.Length);
        ...
        switch (Status)
        {
            case 0:// 序盤
                do{
                    // 序盤のコメントがない人だと人を変える
                    if (C_manager.Audience[NameNum].Comment_1[0] == "")
                    {
                        NameNum = 14;
                        Debug.Log("null発見");
                    }
                    do{
                        ComeNum = Random.Range(0, C_manager.Audience[NameNum].Comment_1.Length);
                    } while (C_manager.Audience[NameNum].Comment_1[ComeNum] == null);
                    Check = C_manager.Audience[NameNum].Name + C_manager.Audience[NameNum].Comment_1[ComeNum];

                    //Debug.Log(Check);
                    //Debug.Log(C_manager.Previous);
                }
                while (Check == C_manager.Previous); // ひとつ前のコメントと同じが調べている
                
                this.GetComponent<Text>().text += C_manager.Audience[NameNum].Name + C_manager.Audience[NameNum].Comment_1[ComeNum];
                C_manager.Previous = this.GetComponent<Text>().text;
                break;
            case 1:// 中盤
                this.GetComponent<Text>().text += C_manager.Audience[NameNum].Comment_2[Random.Range(0, C_manager.Audience[NameNum].Comment_2.Length)];
                break;
            case 2:// 終盤
                this.GetComponent<Text>().text += C_manager.Audience[NameNum].Comment_3[Random.Range(0, C_manager.Audience[NameNum].Comment_3.Length)];
                break;
        }
    }
